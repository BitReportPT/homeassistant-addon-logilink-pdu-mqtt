...
### Fixed
- Fixed entity_id duplication in MQTT Discovery: outlets now appear as switch.rack_01_outlet1 (no more pdu_rack_01_rack_01_outlet_1)
- Home Assistant entity IDs are now clean and predictable for all outlets

## [1.3.3] - 2025-07-15

### Added
- Added version.json file for proper Home Assistant version detection
- Improved repository structure for better add-on store integration

### Fixed
- Fixed version detection issue in Home Assistant Supervisor
- Repository now properly shows available updates

## [1.3.2] - 2025-07-14

### Fixed
- Fixed entity naming to match dashboard expectations
- Added object_id to MQTT Discovery for consistent entity IDs
- Entity IDs now correctly created as switch.rack_01_outlet1 (without extra underscore)
- Dashboard entities should now be found correctly

### Changed
- Simplified entity names in discovery (removed redundant PDU name prefix)

## [1.3.1] - 2025-07-14

### Fixed
- **Critical**: Fixed Dockerfile build error
- Removed references to deleted files (mqtt_bridge.py, pdu_manager.py)
- Docker build now completes successfully

### Changed
- Cleaned up Dockerfile to only copy existing files
